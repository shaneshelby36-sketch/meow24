...
// Pattern confirmed via Kalshi market URLs, e.g.:
//   https://kalshi.com/markets/kxbtc15m/bitcoin-price-up-down/kxbtc15m-26aug251600
// Series tickers are KX{SYMBOL}15M (uppercase). All entries below follow this
...
class TradingBot {
  ...
  _loadKalshiSeriesCacheFromDisk() {
    try {
      ...
      for (const [series, row] of Object.entries(raw || {})) {
        ...
        // Never load a cached market whose window has already closed — that's
        // the exact stale-ticker-on-startup bug that causes market_not_found 404s.
  ...
  async _fetchLiveMarket(seriesTicker, minMsLeft = 1500) {
    ...
    // --- DIAG: log all candidates returned by the series lookup ---
    let _diagCandidates = null;
    if (this.client && typeof this.client.peekOpenMarkets === 'function') {
      _diagCandidates = this.client.peekOpenMarkets(seriesTicker) || [];
    }
    ...
    // Refresh candidate list after fetch (may have just populated from network).
    if (this.client && typeof this.client.peekOpenMarkets === 'function') {
      const fresh = this.client.peekOpenMarkets(seriesTicker) || [];
      if (fresh.length > 0) _diagCandidates = fresh;
    }

    const _nowUtc = new Date().toISOString();
    const _nowMs = Date.now();
    const _winStart = new Date(Math.floor(_nowMs / 900_000) * 900_000).toISOString();
    const _winEnd   = new Date((Math.floor(_nowMs / 900_000) + 1) * 900_000).toISOString();
    console.log(
      `[diag:fetchLiveMarket] series=${seriesTicker} utc=${_nowUtc}` +
      ` window=${_winStart}→${_winEnd}` +
      ` candidates=${JSON.stringify((_diagCandidates || []).map((m) => ({
        ticker: m.ticker,
        event_ticker: m.event_ticker,
        status: m.status,
        open_time: m.open_time,
        close_time: m.close_time,
        expiration_time: m.expiration_time || m.expected_expiration_time,
      })))}` +
      ` selected=${found ? found.ticker : 'null'}`
    );
  ...
  async _openPosition({
    ...
    if (this.config.mode === 'live' && !this._inShadow) {
      ...
      for (let attempt = 0; attempt < maxEntryAttempts; attempt++) {
        ...
        // Re-check entry cutoff on every attempt using current time — the
        // cutoff check in _openPosition used an earlier snapshot of minutesLeft
        // and time passes between evaluation and each order attempt.
        const minutesLeftNow = (closeAt - Date.now()) / 60000;
        const liveCutoff = isModel
          ? modelEntryCutoffMinutes(this.config)
          : Number.isFinite(Number(this.config.minMinutesToOpen))
            ? Number(this.config.minMinutesToOpen)
            : 3;
        if (liveCutoff > 0 && minutesLeftNow < liveCutoff) {
          console.warn(
            `[bot] ${symbol} entry aborted: ${minutesLeftNow.toFixed(2)}m left < ${liveCutoff}m cutoff at attempt ${attempt + 1}`
          );
          break;
        }
      ...
      // --- DIAG: full pre-order snapshot ---
      {
        const _preUtc = new Date().toISOString();
        const _preClose = trade.windowCloseTime
          ? new Date(trade.windowCloseTime).toISOString()
          : 'n/a';
        const _msSinceSelected = trade.openedAt ? Date.now() - trade.openedAt : 'n/a';
        ...
        if (this.client && typeof this.client.getMarket === 'function') {
          try {
            ...
            _directVerify = `error(${_e && _e.message ? _e.message : _e})`;
          }
        }
        console.log(
      ...
      // stale/expired ticker. Bust it now so the next compute cycle re-fetches
      // the live market from Kalshi rather than reusing the dead ticker.
      const allMarketNotFound =
        filled < 1 &&
        lastErr &&
        (Number(lastErr.status) === 404 || /HTTP 404\b/.test(String(lastErr.message))) &&
        /market_not_found/.test(String(lastErr.message));
      if (allMarketNotFound) {
        const seriesTicker = SERIES_BY_SYMBOL[symbol];
        if (seriesTicker) {
          if (this._lastLiveMarket) delete this._lastLiveMarket[seriesTicker];
          if (this._lastLiveMarketAt) delete this._lastLiveMarketAt[seriesTicker];
          if (this.client && typeof this.client.invalidateOpenMarkets === 'function') {
            this.client.invalidateOpenMarkets(seriesTicker);
          }
          console.warn(`[bot] market_not_found on ${symbol} — busted series cache for ${seriesTicker}; will re-fetch next cycle`);
        }
      }

      if (filled < 1) {
        // market_not_found: the window closed mid-flight. No cooldown — the
        // cache is already busted above so the next cycle retries immediately.
        if (allMarketNotFound) this._clearEntryMiss(symbol, side);
        const miss = allMarketNotFound
          ? { streak: 0, cooldownMs: 0 }
          : this._noteEntryMiss(symbol, null, closeAt, side);
        ...
        this.lastError =
          ...
          (allMarketNotFound ? ` — retrying next cycle.` : ` — skipping this ${String(side).toUpperCase()} ${coolLabel} (miss #${miss.streak}); other cryptos/sides still open.`);
        this.lastDecision = this.lastError;
        this._logActivity(this.lastDecision, {
          kind: 'open',
          symbol,
