  ...
  async _request(method, path, opts = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (method === 'POST' && path.includes('orders')) {
        console.log(`[diag:request] POST ${url} keyId=${this.keyId} hasKey=${!!this.privateKey} body=${bodyStr}`);
      }
      ...
      if (method === 'POST' && path.includes('orders') && !res.ok) {
        console.log(`[diag:response] status=${res.status} url=${url} body=${text}`);
      }
      if (res.status === 429) {
        this._noteRateLimit();
